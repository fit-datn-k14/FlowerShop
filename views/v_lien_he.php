<div id="bsm_contact">
  <div class="col-md-12 col-sm-12 col-xs-12 bsm_title_section" style="margin-top: 30px">
    <h1><?=$title_website?></h1>
      <img src="public/img/divider.png" alt="">
  </div>
</div>
<div class="container">
   <div class="col-md-12 col-sm-12 col-xs-12">
       <div class="row">
           <div class="bsm_contact_content">
               <div class="col-md-6 col-sm-6 col-xs-6">
                   <div class="bsm_contact_image">
                       <img src="public/images/adress_icon.png" alt="">
                   </div>
                   <div class="bsm_contact_text">
                       <p><?=$address?></p>
                   </div>
               </div>
               <div class="col-md-6 col-sm-6 col-xs-6">
                   <div class="bsm_contact_image">
                       <img src="public/images/phone_icon.png" alt="">
                   </div>
                   <div class="bsm_contact_text">
                       <p><?=$phone_number?></p>
                   </div>
               </div>
               <div class="col-md-6 col-sm-6 col-xs-6">
                   <div class="bsm_contact_image">
                       <img style="width:40px; color: red" src="public/images/facebook.png" alt="">
                   </div>
                   <div class="bsm_contact_text">
                       <a>May-Shop hoa Hà Nội</a>
                   </div>
               </div>
               <div class="col-md-6 col-sm-6 col-xs-6">
                   <div class="bsm_contact_image">
                       <img style="width:40px; color: red" src="public/images/email.png" alt="">
                   </div>
                   <div class="bsm_contact_text">
                       <a href="mailto:abcxyz@gmail.com">abcxyz@gmail.com</a>
                   </div>
               </div>
            </div>
      </div>
</div>
